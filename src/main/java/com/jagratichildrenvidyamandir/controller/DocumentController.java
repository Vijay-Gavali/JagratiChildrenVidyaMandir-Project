...
public class DocumentController {
	...
	public ResponseEntity<?> upload(@PathVariable Integer userId, @PathVariable DocumentType type,
			@RequestParam("file") MultipartFile file) {
		try {
			DocumentDTO dto = service.uploadDocument(userId, type, file);
			if (dto == null)
				return ResponseEntity.badRequest().body("Invalid user or file");
	...
	/**
	 * New endpoint: create or replace document for user by type. URL: PUT
	 * /api/documents/update/{userId}/{type} Body: form-data with key "file" (File)
	 */
	@PutMapping("/update/{userId}/{type}")
	public ResponseEntity<?> updateDocumentByUserAndType(@PathVariable Integer userId, @PathVariable DocumentType type,
			@RequestParam("file") MultipartFile file) {
		try {
	...
	public ResponseEntity<?> deleteDocument(@PathVariable Integer userId, @PathVariable String type) {
		...
		// Accept cases like "student_aadhar", "STUDENT-AADHAR", "studentAadhar" (we
		...
		try {
			...
			String snake = normalized.replaceAll("([a-z])([A-Z])", "$1_$2") // camel->snake if any
					.replaceAll("[\\s]+", "_");
			...
			try {
				...
				return ResponseEntity.badRequest()
						.body("Unknown document type: '" + type + "'. Allowed: " + String.join(", ",
		...
		if (!deleted) {
			return ResponseEntity.status(404).body("No document found for userId=" + userId + " and type=" + docType);
		}
	...
	/**
	 * Download endpoint: GET /api/documents/download/{userId}/{type} Streams the
	 * file as attachment with the original filename.
	 */
	@GetMapping("/download/{userId}/{type}")
	public ResponseEntity<?> downloadDocument(@PathVariable Integer userId, @PathVariable String type) {
		...
		try {
			...
			String snake = normalized.replaceAll("([a-z])([A-Z])", "$1_$2").replaceAll("[\\s]+", "_");
			snake = snake.toUpperCase(Locale.ROOT);
			try {
				docType = DocumentType.valueOf(snake);
			} catch (IllegalArgumentException e2) {
